...
export default function GroupScoring() {
  ...
  const [localScores, setLocalScores] = useState<{
    [entryId: string]: { [hole: number]: number };
  }>({});
    ...
    const fetchCourseHoles = async () => {
      ...
      try {
        const response = await fetch(
          `/api/courses/${scoringData.tournament.courseId}/holes`,
        );
        if (response.ok) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description:
            "Some changes were overwritten by more recent updates from other devices",
          variant: "default",
        });
      }
    } catch (error) {
  ...
  const fetchGroupScoringData = async (
    tournamentId: string,
    groupId: string,
  ) => {
    try {
      const response = await fetch(
        `/api/tournaments/${tournamentId}/scores/${groupId}`,
      );
      if (response.ok) {
        const data = await response.json();
        setScoringData(data);
  ...
  const updateScore = async (entryId: string, hole: number, change: number) => {
    ...
    try {
      ...
      console.log(
        `Queued score update: entry ${entryId}, hole ${hole}, strokes ${newScore}`,
      );
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-lg">
                {scoringData.group.name}
              </CardTitle>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {scoringData.tournament.name} •{" "}
                {scoringData.tournament.course.name}
              </p>
              {scoringData.group.teeTime && (
                <p className="text-xs text-gray-500">
                  Tee Time:{" "}
                  {new Date(scoringData.group.teeTime).toLocaleTimeString(
                    "en-US",
                    {
                      hour: "2-digit",
                      minute: "2-digit",
            ...
            {/* Sync Status Chip */}
            <div
              className={getSyncStatusClasses(syncStatus)}
            ...
            <div className="min-w-full">
              ...
              <div className="sticky top-0 bg-white dark:bg-gray-900 border-b">
                ...
                <div className="grid grid-cols-22 gap-0 text-center text-xs font-medium">
                  ...
                  <div className="p-2 border-r bg-blue-50 dark:bg-blue-900/20">
                    OUT
                  </div>
                  {Array.from({ length: 9 }, (_, i) => (
                  ...
                  <div className="p-2 border-r bg-green-50 dark:bg-green-900/20">
                    IN
                  </div>
                ...
                <div className="grid grid-cols-22 gap-0 text-center text-xs bg-gray-100 dark:bg-gray-700">
                  <div className="p-1 border-r text-gray-500 dark:text-gray-400">
                    Handicap
                  </div>
                  {Array.from({ length: 9 }, (_, i) => (
                    <div
                      key={i}
                      className="p-1 border-r text-gray-500 dark:text-gray-400"
                    >
              ...
              {scoringData.entries.map((entry) => {
                ...
                const frontNine = Array.from(
                  { length: 9 },
                  (_, i) => playerScores[i + 1] || 0,
                ).reduce((sum, score) => sum + score, 0);
                const backNine = Array.from(
                  { length: 9 },
                  (_, i) => playerScores[i + 10] || 0,
                ).reduce((sum, score) => sum + score, 0);
                const totalScore = frontNine + backNine;

                // Calculate par totals for played holes only
                const frontNineHolesPlayed = Array.from(
                  { length: 9 },
                  (_, i) => playerScores[i + 1] || 0,
                ).filter((score) => score > 0).length;
                const backNineHolesPlayed = Array.from(
                  { length: 9 },
                  (_, i) => playerScores[i + 10] || 0,
                ).filter((score) => score > 0).length;

                const frontParPlayed =
                  courseHoles.length === 18
                    ? Array.from({ length: 9 }, (_, i) =>
                        playerScores[i + 1] > 0 ? courseHoles[i]?.par || 4 : 0,
                ...
                const frontToPar =
                  frontNine && frontNineHolesPlayed > 0
                    ? frontNine - frontParPlayed
                    : 0;
                const backToPar =
                  backNine && backNineHolesPlayed > 0
                    ? backNine - backParPlayed
                    : 0;
                const totalToPar =
                  totalScore && frontNineHolesPlayed + backNineHolesPlayed > 0
                    ? totalScore - totalParPlayed
                    : 0;
                ...
                const netToPar =
                  netTotal && frontNineHolesPlayed + backNineHolesPlayed > 0
                    ? netTotal - totalParPlayed
                    : 0;

                return (
                  <div
                    key={entry.id}
                    className="grid grid-cols-22 gap-0 border-b"
                  >
                    {/* Player Info */}
                    ...
                    {Array.from({ length: 9 }, (_, holeIndex) => {
                      ...
                      const strokesRcvd =
                        courseHoles.length === 18 && showHandicapDots
                          ? strokesReceived(
                              entry.playingCH,
                              courseHoles[holeIndex]?.strokeIndex || 1,
                            )
                          : 0;
                    ...
                    <div className="border-r p-2 flex flex-col items-center justify-center bg-blue-50 dark:bg-blue-900/20">
                      ...
                      {frontNine > 0 && (
                        <div
                          className={`text-xs tabular-nums ${frontToPar === 0 ? "text-gray-500" : frontToPar > 0 ? "text-red-600" : "text-green-600"}`}
                        >
                          {frontToPar === 0
                            ? "E"
                            : frontToPar > 0
                    ...
                    {Array.from({ length: 9 }, (_, holeIndex) => {
                      ...
                      const strokesRcvd =
                        courseHoles.length === 18 && showHandicapDots
                          ? strokesReceived(
                              entry.playingCH,
                              courseHoles[holeIndex + 9]?.strokeIndex || 1,
                            )
                          : 0;
                    ...
                    <div className="border-r p-2 flex flex-col items-center justify-center bg-green-50 dark:bg-green-900/20">
                      ...
                      {backNine > 0 && (
                        <div
                          className={`text-xs tabular-nums ${backToPar === 0 ? "text-gray-500" : backToPar > 0 ? "text-red-600" : "text-green-600"}`}
                        >
                          {backToPar === 0
                            ? "E"
                            : backToPar > 0
                    ...
                    <div className="p-2 flex flex-col items-center justify-center bg-yellow-50 dark:bg-yellow-900/20">
                      ...
                      {totalScore > 0 && (
                        <div
                          className={`text-sm tabular-nums ${totalToPar === 0 ? "text-gray-500" : totalToPar > 0 ? "text-red-600" : "text-green-600"}`}
                        >
                          {totalToPar === 0
                            ? "EVEN"
                            : totalToPar > 0
              ...
              <div className="grid grid-cols-22 gap-0 text-center text-xs bg-gray-50 dark:bg-gray-800 border-t-2">
                <div className="p-1 border-r text-gray-600 dark:text-gray-400 font-medium">
                  Par
                </div>
                {Array.from({ length: 9 }, (_, i) => (
                  <div
                    key={i}
                    className="p-1 border-r text-gray-600 dark:text-gray-400"
                  >
                    {courseHoles[i]?.par || 4}
                  </div>
                ))}
